...
| 070 | 2026-03-11 | HU-070 | Auth Guard: Proteger acceso directo a páginas de Creador (/pipelines, etc) | feature | QUALITY | DONE (implementado en `middleware.ts:72-77`; `/pipelines` lo cerro WKH-AUDIT-V2 en 220d5ce1, no la propia HU — auditoria 2026-08-19) | 7017063 + 220d5ce1 (main) |
...
| 076 | 2026-07-08 | WKH-162 | Config-drift de la address del marketplace — voucher EIP-712 vs server path (subset urgente WKH-130) | security | FAST+AR | DONE (mergeado; `src/lib/contracts/marketplaceAddressCoherence.ts` + usado en `src/app/api/creator/earnings/voucher/route.ts:35`) | fix/076-wkh-162-marketplace-address-config-drift (43a01c4) |
| 077 | 2026-08-18 | WKH-361 | Los headers de contracting no atraviesan el proxy de v2 (capa 2 de WKH-360 con cobertura cero en el camino real) + no hay forma de saber qué ambiente delega | bugfix | QUALITY | DONE — F4 APROBADO y mergeado a main. **Aprobado "sin desplegar"**: verificar deploy antes de darlo por vivo | feat/077-headers-proxy ([f4-report.md](077-wkh-361-cutover-a2a-no-cableado/f4-report.md)) |

<!-- Filas recuperadas por la auditoria de indice 2026-08-19: existian como carpeta en doc/sdd/
     pero no tenian fila. Se agregan al final para no desplazar la numeracion existente.
     Detalle y evidencia: doc/sdd/_AUDITORIA-INDICE-2026-08-19.md -->
| 063 | 2026-03-08 | HU-063 | Withdraw Agent Key directo — ruta `src/app/api/agent-keys/[id]/withdraw/route.ts` | feature | QUALITY | DONE (faltaba fila; carpeta `063-withdraw-key-direct`) | main |
| 066 | 2026-03-08 | HU-066 | UI Polish — Navbar, Refresh, Wallet Display & Mobile Home | improvement | full | NO VERIFICABLE — el SDD quedo en `SPEC_APPROVED pending` y no hay story-file cerrado; parte 1 (DepositModal/onSuccess) existe en `src/app/[locale]/agent-keys/page.tsx:47` | — (carpeta `066-ui-polish`) |
| 069b | 2026-03-10 | HU-069 | Creator Wallet Unification — `creator_profiles.wallet_address` | improvement | full | DONE (faltaba fila; ver `src/features/agents/components/UpgradeOnChainButton.tsx:21`) | main (carpeta `069-creator-wallet-unification`) |
| 071 | 2026-03-11 | HU-071 | Remover thirdweb del stack de wallets | refactor | QUALITY | DONE (faltaba fila; `thirdweb` ya no esta en `package.json`, residuo intencional en `src/lib/wallet-cleanup.ts:4`) | main (carpeta `071-remove-thirdweb`) |

...
vi.mock('@/lib/contracts/marketplaceClient', () => ({
  ...
  // requested amount (preserves existing deposited_usdc assertions).
  getDepositedUsdcFromReceipt: vi.fn(async () => null),
}))

// TB-06: idempotent credit helper delegates to supabaseRpc so the existing
// "budget RPC fails → still 200" test continues to drive behavior via supabaseRpc.

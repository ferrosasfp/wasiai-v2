...
export async function getModels({
  ...
  let query = supabase
    .from('agents')
    .select(`${AGENT_PUBLIC_COLUMNS}, creator:creator_profiles(id, username, display_name, avatar_url, verified)`, { count: 'exact' })
    .eq('status', 'active')
  ...
  return { models: (data as unknown as Model[]) ?? [], total: count ?? 0 }
}
...
export async function getModelBySlug(slug: string): Promise<Model | null> {
  ...
  const { data, error } = await supabase
    .from('agents')
    .select(`${AGENT_PUBLIC_COLUMNS}, creator:creator_profiles(id, username, display_name, avatar_url, verified, bio)`)
    .eq('slug', slug)
    .eq('status', 'active')
    .single()
...
export async function getFeaturedModels(limit = 6): Promise<Model[]> {
  ...
  const { data, error } = await supabase
    .from('agents')
    .select(`${AGENT_PUBLIC_COLUMNS}, creator:creator_profiles(id, username, display_name, avatar_url, verified)`)
    .eq('status', 'active')
    .eq('is_featured', true)
    .limit(limit)

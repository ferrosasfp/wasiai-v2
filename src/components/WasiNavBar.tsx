          ...
          <div className="hidden items-center gap-1 sm:flex flex-1" role="list">
            {primaryLinks.map(({ path, label }) => (
              <Link
                ...
                className={`rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${isActive(path)
            ...
            {isLoggedIn && (
              <div ref={createRef} className="relative">
                <button
                  ...
                  className={`flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${createOpen ? 'bg-avax-50 text-avax-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                    }`}
                >
                  {tNav('create')}
            ...
            {/* Trailing links — public (Docs) */}
            {trailingLinksPublic.map(({ path, label }) => (
              <Link
                key={path}
                href={`/${locale}${path}`}
                className={`rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${isActive(path)
            ...
            {loading ? (
              ...
              <div ref={meRef} className="relative">
                <button
                  ...
                  className={`flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-sm font-medium transition-colors ${meOpen ? 'bg-gray-50 text-gray-900' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                    }`}
                >
